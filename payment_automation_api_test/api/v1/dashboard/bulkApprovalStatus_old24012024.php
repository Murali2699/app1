...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $trackingId = !empty($_POST['trackingId']) ? $_POST['trackingId'] : 0;
        $sqlQuery ="select COALESCE(total_count, 0) as total_count,COALESCE(updated_count, 0) as updated_count,COALESCE(status, 2) as status from fn_payment_getbulkapprovalstatus('".$trackingId."');";
        $action ='';
