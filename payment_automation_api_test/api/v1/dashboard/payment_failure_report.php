...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($getSchemeIDqueryStmt !== false) {
      ...
      $sql= "SELECT * FROM fn_payment_getpaymentfailedbeneficiarydetails(:schemeid, :schemename, :subschemename, :idepartment, :isubdepartment, :iyyyymm, :paymentdate, :userid)"; 
    ...
    if ($prepost_count >= 1) {
        ...
        http_response_code(200);
        $data = array("success" => 0, "message" => "Payment failed details were not found for the selection", 'data' => []);
        echo json_encode($data);
        exit();
