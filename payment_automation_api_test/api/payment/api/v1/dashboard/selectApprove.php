...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // if ($schemeID!=0) {
        ...
        // $sqlQuery = "select id,scheme_id as schemeid,yyyymm,benificiary_id as benificiaryid,installment_id::INTEGER as installmentid from fn_payment_getbenificiariesforapproval($benificiaryid,$schemeID,'".$schemename."','".$subschemename."',$idepartment,$isubdepartment,$districtcode,$talukcode,'".$jurisdictionlayercode."','".$iyyyymm."','".$fromdate."','".$todate."',$paymentperiod,$approvalstatus) WHERE id IN (".$whereIn_Ids.");";
        // echo $sqlQuery;exit;
        ...
        // if ($prepost_count >= 1) {
            ...
            $jsonData = json_encode($groupOfArrCombine,true);
                ...
                if($result){
                    ...
                    $data = array(
                        "success" => 0, 
                        "message" => "Payment Pending Not Successfully"
                    );
                }
