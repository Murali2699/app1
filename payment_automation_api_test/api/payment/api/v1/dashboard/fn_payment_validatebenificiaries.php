...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $qry="";
    if($searchValue!=""){
        $qry="where lower(duplicate_aadhaar_present) like '%$searchValue%'
        OR lower(duplicate_account_present) like '%$searchValue%'
        OR lower(invalid_benificiary_name) like '%$searchValue%'
        OR lower(invalid_iin_or_ifsc) like '%$searchValue%'
        OR lower(district_name) like '%$searchValue%'
        OR lower(taluk_name) like '%$searchValue%'
        OR lower(invalid_aadhaar) like '%$searchValue%'";  
    }
    ...
    // $sqlQuery ="select * from fn_payment_validatebenificiaries($schemeID, $iyyyymm,$uploadreferenceno,'".$icreatedby."','".$icreatedfrom."') $qry;";

    $sqlQuery ="select * from fn_validation_getvalidationsummary($schemeID,'".$schemename."','".$subschemename."',$idepartment,$isubdepartment,$districtcode,$talukcode,'".$jurisdictionlayercode."','".$iyearcode."','".$fromdate."','".$todate."',$paymentperiod,'".$user_id."') $qry;";
