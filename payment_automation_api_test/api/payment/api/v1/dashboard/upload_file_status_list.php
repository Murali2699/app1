...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $qry="";
    if($searchValue!=""){
        $qry="where lower(scheme_code) like '%$searchValue%'
        OR lower(file_name) like '%$searchValue%'
        OR lower(scheme_name) like '%$searchValue%'
        OR lower(scheme_code) like '%$searchValue%'
        OR lower(upload_remarks) like '%$searchValue%'";
    }
    ...
    if($columnIndex!=0){
        ...
        $sqlQuery = "select * from fn_payment_getinputfiledetails($schemeID,'".$schemename."','".$subschemename."',$idepartment,$isubdepartment,'".$paymentfromdate."','".$paymenttodate."') $qry;";
        ...
        if ($prepost_count >= 1) {
            $uploadFileList = $queryStmt->fetchAll(PDO::FETCH_ASSOC);
