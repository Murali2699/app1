...
if (!empty($_POST['fileters'])) {
    $arrCase = [
        "filter_scheme_code" => "scheme_code like",
    ...
    foreach ($_POST['fileters'] as $filter) {
        if (array_key_exists($filter['case'], $arrCase)) {
            $whereConditions[] = "{$arrCase[$filter['case']]} '{$filter['inputValue']}'";
        }
    }
}
...
if (isset($district_code) && isset($taluk_code) && isset($shop_code)) {
    $sqlQuery = "SELECT * FROM fn_scheme_getschemes() $addWhere $whereClause $orAdd $qry $ord LIMIT $rowperpage OFFSET $row;";
    // echo $sqlQuery ;exit;
    $countQuery = "SELECT COUNT(*) FROM fn_scheme_getschemes() $addWhere $whereClause $orAdd $qry;";
