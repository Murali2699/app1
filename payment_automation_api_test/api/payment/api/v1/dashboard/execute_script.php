...
if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET["script"])) {
    ...
    if ($scriptName === "generate_payment_file_cnrt" || $scriptName === "transfer_payment_file" || $scriptName === "final_recive_file1") {
        ...
        $rawString = str_replace('"','',$_GET['fileters']);
            ...
            $approvalstatus = isset($_GET['approveStatus']) ? $_GET['approveStatus']:0;
        ...
        // echo $result;
            if($scriptName === "generate_payment_file_cnrt"){
                $data = array(
                "success" => 1,
                "message" => "Generate Payment File Successfully",
                "data"=>$result
                );
            }
            if($scriptName === "transfer_payment_file"){
                ...
                if(!empty($decodeJson) && $decodeJson['status']==1){
                    $data = array(
                    "success" => 1,
                    "message" => "Transfer Payment File to Bank Successfully",
                    "data"=>$result,
                    "datadecode"=> $decodeJson
                    );
                }else{
                    $data = array(
                    "success" => 0,
                    "message" => "Transfer Payment File to Bank Failed, Please Try Again",
                    "data"=>$result,
                    "datadecode"=> $decodeJson
                    );
                }
