...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    /*form filter datas*/
        ...
        $paymentdate = isset($_POST['paymentDate']) ? $_POST['paymentDate']:"2000-01-01";
        ...
        if ($schemeIDprepost_count >= 1) {
            ...
            // if ($schemeID!=0) {
            //  echo "CALL sp_payment_emotransferbulk(false,".$approvalstatus.",$approvedy,".$benificiaryid.", ".$schemeID.",'".$schemename."','".$subschemename."',".$idepartment.",".$isubdepartment.",".$summaryDistrictCode.",".$summaryTalukCode.",'".$jurisdictionlayercode."','".$iyearcode."','".$fromdate."','".$todate."',".$paymentperiod.",".$trackingId.",'$reason_code')";exit;
            // print_r($_POST);exit;
        $action ='';
        $updatestatus=false;
                ...
                $write_db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                    ...
                    $stmt = $write_db->prepare("CALL sp_payment_emotransferbulk(:updatestatus, :approvalstatus, :approvedy, :benificiaryid, :schemeid, :schemename, :subschemename, :idepartment, :isubdepartment, :districtcode, :talukcode, :jurisdictionlayercode, :iyearcode, :fromdate, :todate, :paymentperiod, :trackingid,:reasoncode)");
                ...
                if($result){
                    $data = array(
                    "success" => 1, 
                    "message" => "EMO Bulk Transfer Successfully", 
                    'data' => $result
                    );
                }else{
                    $data = array(
                        "success" => 0, 
                        "message" => "EMO Bulk Transfer Not Successfully",
                        'data' => null
                    );
                }
