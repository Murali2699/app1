...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    /*form filter datas*/
        ...
        // if ($schemeID!=0) {
        //  echo "CALL sp_scheme_createpayment(false,".$approvalstatus.",0,".$benificiaryid.", ".$schemeID.",'".$schemename."','".$subschemename."',".$idepartment.",".$isubdepartment.",".$summaryDistrictCode.",".$summaryTalukCode.",'".$jurisdictionlayercode."','".$iyearcode."','".$fromdate."','".$todate."',".$paymentperiod.",".$trackingId.")";exit;
        ...
        $action ='';
                ...
                $stmt = $write_db->prepare("CALL sp_scheme_createpayment(:paymentid, :ischemeid, :ipaymentcycle, :ipaymentperiod, :iinstallment, :iyyyymm)");

                $stmt->bindParam(':paymentid', $defultIntValue, PDO::PARAM_INT);
                $stmt->bindParam(':ischemeid', $schemeID, PDO::PARAM_INT);
                $stmt->bindParam(':ipaymentcycle', $ipaymentcycle, PDO::PARAM_INT);
                $stmt->bindParam(':ipaymentperiod', $ipaymentperiod, PDO::PARAM_INT);
                $stmt->bindParam(':iinstallment', $iinstallment, PDO::PARAM_INT);
                $stmt->bindParam(':iyyyymm', $iyyyymm, PDO::PARAM_INT);
