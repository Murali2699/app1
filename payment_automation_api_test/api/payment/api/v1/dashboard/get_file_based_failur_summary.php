...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if($taluk_enable == 1){
        $sqlQuery = "select * from public.view_payment_getkmut_summary where status = 'Failure' $qry  $ord limit ".$rowperpage." offset ".$row.";";
        $totalCountsqlQuery = "select COUNT(*) from public.view_payment_getkmut_summary where status = 'Failure' $qry;";
    }else{
        $sqlQuery = "select district_code,district_name,payment_mode,status,reasoncode,remarks,SUM(beneficiaries) as beneficiaries,SUM(amount) as amount from public.view_payment_getkmut_summary where status = 'Failure' $qry GROUP BY district_code,district_name,reasoncode,remarks,payment_mode,status $ord limit ".$rowperpage." offset ".$row.";";
