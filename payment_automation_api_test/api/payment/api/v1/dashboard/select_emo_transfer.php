...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    /*form filter datas*/
        ...
        // $sqlQuery = "select id,scheme_id as schemeid,yyyymm,benificiary_id as benificiaryid,installment_id::INTEGER as installmentid from fn_payment_getbenificiariesforapproval($benificiaryid,$schemeID,'".$schemename."','".$subschemename."',$idepartment,$isubdepartment,$districtcode,$talukcode,'".$jurisdictionlayercode."','".$iyearcode."','".$fromdate."','".$todate."',$paymentperiod,$approvalstatus,$limit,0) WHERE id IN (".$whereIn_Ids.");";

        // $sqlQuery = "select id,scheme_id as schemeid,yyyymm,benificiary_id as benificiaryid,installment_id::INTEGER as installmentid from fn_payment_getbenificiariesforapproval($benificiaryid,$schemeID,'".$schemename."','".$subschemename."',$idepartment,$isubdepartment,$districtcode,$talukcode,'".$jurisdictionlayercode."','".$yearString."','".$fromdate."','".$todate."',$paymentperiod,$approvalstatus,$limit,0,'".$reason_code."') WHERE id IN (".$whereIn_Ids.");";
        ...
        $action ='';
            ...
            // echo $jsonData;exit; 
                ...
                // echo "CALL sp_payment_emotransferselected(false, '$jsonData', '$approvedy')"; exit;
                // echo "CALL sp_payment_emotransferselected(false, '$jsonData', $approvedy)";exit;
                $write_db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

                $stmt = $write_db->prepare("CALL sp_payment_emotransferselected(:updatestatus, :benificiarydata, :approvedy)");
                ...
                if($result){
                    ...
                    $data = array(
                    "success" => 1, 
                    "message" => "Tranfered to EMO Successfully", 
                    'data' => $result
                    );
                }else{
                    $data = array(
                        "success" => 0, 
                        "message" => "Tranfered to EMO Not Successfully"
                    );
                }
