...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $user_id = isset($_POST['user_id']) ? $_POST['user_id'] : 0;
        ...
        // $ddl_payment_period = isset($_POST['ddl_payment_period']) ? $_POST['ddl_payment_period'] : '';
            // $whereColumn = isset($_POST['whereColumn']) ? $_POST['whereColumn'] : '';
        
        /* get scheme id   fn_scheme_getschemesbyuser(".$user_id.")*/
        if($schemecate =='' && $schemecode =='' && $schemename =='' && $subschemename =='' && $idepartment =='' && $isubdepartment ==''){
        ...
        elseif($schemecate !='' && $schemecode =='' && $schemename !='' && $subschemename !='' && $idepartment !='' && $isubdepartment !=''){
            // echo '1';exit;
        $getSchemeIDsqlQuery = "select id from fn_scheme_getschemesbyuser(".$user_id.") where scheme_category like '".$schemecate."' and scheme_name like '".$schemename."' and subscheme_name like '".$subschemename."' and   department = '".$idepartment."' and subdepartment ='".$isubdepartment."';";
        }
        elseif($schemecate !='' && $schemecode =='' && $schemename =='' && $subschemename !='' && $idepartment !='' && $isubdepartment !=''){
            // echo '1';exit;
        $getSchemeIDsqlQuery = "select id from fn_scheme_getschemesbyuser(".$user_id.") where scheme_category like '".$schemecate."'  and subscheme_name like '".$subschemename."' and   department = '".$idepartment."' and subdepartment ='".$isubdepartment."';";
        }
        else{
            // echo '345';exit;
        ...
        $sqlQuery = "select * from fn_scheme_getschemesbyid($schemeID,$user_id);";
        // echo $sqlQuery; exit;
        $action ='';
